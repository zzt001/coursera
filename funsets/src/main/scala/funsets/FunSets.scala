...
trait FunSets extends FunSetsInterface {
   ...
   */
  def filter(s: FunSet, p: Int => Boolean): FunSet = (elem => contains(s, elem) && p(elem))
  ...
  def forall(s: FunSet, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      if (a > bound) true
      else if (contains(filter(s, x => !p(x)), a)) false
      else iter(a + 1)
    }
    iter(-bound)
  }
   ...
   */
  def map(s: FunSet, f: Int => Int): FunSet = elem => exists(s, x => f(x) == elem)
